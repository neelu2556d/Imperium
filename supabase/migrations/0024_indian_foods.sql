-- ---------- indian_foods (Fuel search: IFCT-style Indian food database) ----------
-- A public, read-only reference table the Fuel food search reads from ("Indian
-- DB" badge). No user_id / RLS: everyone searches the same catalogue. Per-food
-- macro tweaks live in `user_overrides` (jsonb) so a corrected value survives
-- future searches (see /api/food-edit). Run this file in the Supabase SQL editor.
...
ON indian_foods USING gin(to_tsvector('english', name));
...
('Palak / Spinach (cooked)','Vegetables',26,2.0,0.7,2.9,2.0),
